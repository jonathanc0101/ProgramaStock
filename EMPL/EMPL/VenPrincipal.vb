...
Public Class venPrincipal
    ...
    Private listadoObjetos As ObjetosDeNegocio.empleados
    Private Configuracion As Configuracion.empleados
    Private listaFiltrada As CustomSortableBindingList(Of ObjetosDeNegocio.empleado)


    Sub New(ByRef ParamListado As ObjetosDeNegocio.empleados, ByRef ParamCfg As Configuracion.empleados)
    ...
    Private Sub venPrincipal_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.Text = "Empleados/as"
        RegistroToolStripMenuItem.Text = "Empleado/a"
...
#Region "Metodos"
    Private Sub buscar(paramGuardarEstado As Boolean)
        ...
        listaFiltrada = New CustomSortableBindingList(Of ObjetosDeNegocio.empleado)

        If CustomTextBoxBuscador.Text = "" Then
            listaFiltrada.AddRange(listadoObjetos.Lista)
        Else
            listaFiltrada.AddRange(listadoObjetos.Lista.Where(Function(x) x.nombre.ToLower Like "*" & CustomTextBoxBuscador.Text.ToLower & "*" _
    ...
    Private Sub nuevoRegistro()
        ComunicacionProcesos.empleado.Nueva()
    End Sub
    ...
    Private Sub editarRegistro()
        ...
        If grilla.SelectedRows.Count > 0 Then
            For Each row As DataGridViewRow In grilla.SelectedRows
                ComunicacionProcesos.empleado.EditarPorID(row.Cells("idEmpleadoDataGridViewTextBoxColumn").Value)
            Next
        End If
    ...
    Private Sub eliminarRegistro()
        If grilla.SelectedRows.Count > 0 Then
            ...
            Else
                mensaje = "¿Desea eliminar el registro con referencia: " & grilla.SelectedRows(0).Cells.Item("NombreDataGridViewTextBoxColumn").Value.ToString & "? "
            End If

            If MessageBox.Show(mensaje, "Empleado/a", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                For Each row As DataGridViewRow In grilla.SelectedRows
                    Try

                        listadoObjetos.eliminar(row.Cells("idEmpleadoDataGridViewTextBoxColumn").Value)

                    Catch ex As Exception

                        MessageBox.Show(ex.Message, "Empleado/a", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    End Try
                Next
    ...
    Private Function obtenerID() As String
        Return "empleado " & Me.grilla.CurrentRow.Cells("idEmpleadoDataGridViewTextBoxColumn").Value
    End Function
    ...
    Private Sub ponerIconos()
        ...
        ToolStripButtonEditar.Image = Iconos.My.Resources.EditarIMG
        ...
        If info.Type = DataGridViewHitTestType.Cell And e.Button = Windows.Forms.MouseButtons.Left Then
            ...
            Else
                If Not (Control.ModifierKeys = Keys.Shift Or Control.ModifierKeys = Keys.Control) And grilla.SelectedRows.Count <= 1 Then
                    Me.grilla.CurrentCell = grilla.Rows(info.RowIndex).Cells("idEmpleadoDataGridViewTextBoxColumn")
                End If
...
#Region "Eventos"
    ...
    Private Sub ToolStripButton1_Click(sender As Object, e As EventArgs) Handles ToolStripButtonNuevo.Click
        nuevoRegistro()
    End Sub

    Private Sub ToolStripButton2_Click(sender As Object, e As EventArgs) Handles ToolStripButtonEditar.Click
        editarRegistro()
    End Sub

    Private Sub ToolStripButton3_Click(sender As Object, e As EventArgs) Handles ToolStripButtonEliminar.Click
        eliminarRegistro()
    End Sub
